    ...
    public function insert($tabela = null, $data = null, $get_last_id = null)
    {
        if ($tabela && is_array($data)) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $this->session0->set_flashdata('error', 'Erro ao salvar dados');
            }
        } else {
            return FALSE;
        }
        ...
        if ($tabela && is_array($condicao)) {
            ...
            if (!$status) {
                foreach ($error as $code) {
                    //1451 = erro SQL: Esse registro não poderá ser deletado, pois está sendo utilizado em outra tabela.
                    if ($code == 1451) {
                        $this->session->set_flashdata('error', 'Esse registro não poderá ser deletado, pois está sendo utilizado em outra tabela.');
                    }
                }
            } else {
